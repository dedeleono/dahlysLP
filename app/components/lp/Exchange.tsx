...
export default function Exchange() {
    ...
    const trtnField = (
        <label className="flex flex-row items-center pb-5 relative">
            <img className="w-12 pr-2 scale-[2] pt-1" src="/images/trtn.png"/>
            <div className="w-16 text-xl pr-[0.66rem] ">
                <strong>IMGN</strong>
            </div>
            <input
                type="number"
    ...
    return (
        <div>
            ...
            />
            <div>
                <h2 className="font-damion tracking-wide text-4xl" style={{textShadow: '0 0 3px #484747'}}>Exchange</h2>
                <p className="text-base py-1">
                    Buy $IMGN to deposit in the Dreams Liquidity Pool
                </p>
                ...
                <button
                    className="btn group bg-white bg-opacity-30 border-0 btn-circle btn-lg  ml-8 py-2 my-2"
                    onClick={() => {
                        changeSwapType();
                    }}
                >
                ...
                <div className="mt-4 gap-2">
                    <button
                        className={`btn rounded-lg btn-block btn-lg relative overflow-hidden shadow ${isPending ? 'loading' : ''} ${!swapAmounts.trtn ? 'btn-disabled' : 'bg-gray-800 border-none hover:text-[#d5c291] hover:scale-105'}`}
